...
### 4. DocuSign Navigator Agreement アップロード（linked_data紐付け）

Salesforceレコードの添付PDFをDocuSign Navigatorの合意書リポジトリにアップロードし、`linked_data`（`application_name`/`object_name`/`record_id`のtriplet）を付与することで、Salesforce標準の「Docusign Completed Agreements」コンポーネントにそのドキュメントを表示させる機能です。商談ページ用LWCとAgentforce向けアクションの両方を用意しています。

| コンポーネント | 種別 | 役割 |
|---|---|---|
| `NavigatorAgreementUploadService` | Apexクラス | Navigator Bulk Upload API（Data In）呼び出し共通サービス |
| `NavigatorAgreementUploadController` | Apexクラス | LWC向け `@AuraEnabled` メソッド群 |
| `navigatorAgreementUploader` | LWC | 商談レコードページ配置用アップロードUI |
| `UploadAgreementToNavigatorAction` | Apexクラス | Agentforce向け `@InvocableMethod` |

**主な機能：**
- レコードの添付ファイル一覧から1件選択してアップロード
- Bulk Upload Job作成 → 署名付きAzure Blob Storage URLへ直接PUT → Job完了マーク、の3ステップを1メソッドで実行
- アップロード時に `x-ms-meta-metadata` ヘッダーへ `linked_data: [{ application_name: "Salesforce", object_name, record_id }]` をJSON文字列で埋め込み、ドキュメントを対象レコードに紐付け
- `object_name` はレコードIDから `getSObjectType().getDescribe().getName()` で自動判定（商談・取引先など任意のオブジェクトに対応）
- Agentforceから「〇〇のファイルをNavigatorにアップロードして」で呼び出し可能
- 認証は既存の `NavigatorService.loadConfig()` / `getAccessToken()` とNamed Credential（`DocuSign_Navigator`）をそのまま再利用。既存のApexクラス・LWCは変更していない
- Azure Blob Storage（デモ環境）への直接アップロード用に Remote Site Setting を4件追加（プライマリ/セカンダリ×2リージョン）
...
│   └── navigatorAgreementUploader  ← Navigatorへのドキュメントアップロード（商談ページ）
│
├── Apex Controllers（@AuraEnabled）
│   ├── NavigatorController
│   ├── WorkspaceController
│   ├── DocuSignEnvelopeController
│   └── NavigatorAgreementUploadController
│
├── Apex Services（API呼び出し）
│   ├── NavigatorService                  ← JWT認証 + Navigator/Workspaces API共通
│   ├── WorkspaceService                  ← Workspaces API個別操作
│   └── NavigatorAgreementUploadService   ← Navigator Bulk Upload API（Data In + linked_data）
│
└── Agentforce Invocable Actions（@InvocableMethod）
    ...
    ├── UploadWorkspaceDocumentAction      ← ドキュメントアップロード（Workspaces）
    └── UploadAgreementToNavigatorAction   ← ドキュメントアップロード（Navigator linked_data）
```
...
- `navigatorAgreementUploader`：Navigatorへのドキュメントアップロードパネル
